    ...
    if turn=="Player2":
        ...
        for i in range(0,4):
            ...
            if col1==True and players_r[shomare].y+50>=players_b[i].y:
                ...
                ki="Player1"
                chandom=i
    return(vx,vy,k2,h2,barkhordki,chandom,ki)
def barkhordkiy(players_r , players_b,turn,shomare,vx,vy,k2,h2,player_r_v,player_b_v):
    if turn=="Player2":
        if players_r[shomare].x+100>=1280 or players_r[shomare].x<=0:
            vx=-vx
    if turn=="Player1":
...
def barkhord_g(ball1,goal):
    if ball1.tx<=0 and 275<=ball1.ty<=500:
        goal=1
    if ball1.tx>=1230 and 275<=ball1.ty<=500:
        goal=2
    return goal
#ورودي ها
s=0
...
while EZS==0:
    ...
    while flag_from==1:
        ...
        w_font=font.render("Music",True,(156, 149, 149))
        disp.blit(w_font,(615,260))
        ary_font=font1.render("Arya Mohammadi",True,(255,255,255))
        disp.blit(ary_font,(570,290))
    ...
    while not done:   
        ...
        if nobat=="Player2" and flag==1 and flag_t==0:
            ...
            if flag==1 and players_r[shomare].y>yp and flag_t==0:
                players_r[shomare].y=players_r[shomare].y-players_rv[shomare].vy
            if flag_ki==0 and flag==1:
                players_rv[shomare].vx,players_rv[shomare].vy,k2,h2,barkhordki,chandom,ki=barkhord(players_r , players_b,nobat,shomare,players_rv[shomare].vx,players_rv[shomare].vy,k2,h2,player_r_v,player_b_v)
            if flag_ki==1 and flag==1:
                players_rv[shomare].vx,players_rv[shomare].vy,k2,h2=barkhordkiy(players_r , players_b,nobat,shomare,players_rv[shomare].vx,players_rv[shomare].vy,k2,h2,player_r_v,player_b_v)
            toop_gg=barkhord_t_m(players_r,players_b,ball1,nobat,shomare)
            if barkhordki=="true":
                flag_ki=1
            if flag_ki==1 and ki=="Player2":
                if (to!=k2 and tki==0) or (to+1!=k2 and tki==0) or (to-1!=k2 and tki==0):
                    if players_r[shomare].x<xp:
                        players_rv[chandom].vx=1
                    if players_r[shomare].x>xp:
                        players_rv[chandom].vx=-1
                    if players_r[shomare].x==xp:
                        players_rv[chandom].vx=0
                if (lo!=h2 and tki==0) or (lo+1!=h2 and tki==0) or (lo-1!=h2 and tki==0):
                    if players_r[shomare].y<yp:
                        players_rv[chandom].vy=1
                    if players_r[shomare].y>yp:
                        players_rv[chandom].vy=-1
                    if players_r[shomare].y==yp:
                        players_rv[chandom].vy=0
                if tki!=110:
                    tki=tki+1
                    players_r[chandom].x=players_r[chandom].x+players_rv[chandom].vx
                    players_r[chandom].y=players_r[chandom].y+players_rv[chandom].vy
                if tki>=2:
                    players_rv[chandom].vx,players_rv[chandom].vy,k2,h2=barkhordkiy(players_r , players_b,ki,chandom,players_rv[chandom].vx,players_rv[chandom].vy,k2,h2,player_r_v,player_b_v)
                toop_gg=barkhord_t_m(players_r,players_b,ball1,ki,chandom)
                if tki==110:
                    flag=0
                    flag1=0
                    flag2=0
                    ...
                    chandom=0
                    barkhordki="false"
            if flag_ki==1 and ki=="Player1":
                if (to!=k2 and tki==0) or (to+1!=k2 and tki==0) or (to-1!=k2 and tki==0):
                    if players_b[shomare].x<xp:
                        players_bv[chandom].vx=1
                    if players_b[shomare].x>xp:
                        players_bv[chandom].vx=-1
                    if players_b[shomare].x==xp:
                        players_bv[chandom].vx=0
                if (lo!=h2 and tki==0) or (lo+1!=h2 and tki==0) or (lo-1!=h2 and tki==0):
                    if players_b[shomare].y<yp:
                        players_bv[chandom].vy=1
                    if players_b[shomare].y>yp:
                        players_bv[chandom].vy=-1
                    if players_b[shomare].y==yp:
                        players_bv[chandom].vy=0
                if tki!=110:
                    tki=tki+1
                    players_b[chandom].x=players_b[chandom].x+players_bv[chandom].vx
                    players_b[chandom].y=players_b[chandom].y+players_bv[chandom].vy
                if tki>=2:
                    players_bv[chandom].vx,players_bv[chandom].vy,k2,h2=barkhordkiy(players_r , players_b,ki,chandom,players_bv[chandom].vx,players_bv[chandom].vy,k2,h2,player_r_v,player_b_v)
                toop_gg=barkhord_t_m(players_r,players_b,ball1,ki,chandom)
                if tki==110:
                    flag=0
                    flag1=0
        ...
        if nobat=="Player1" and flag==1 and flag_t==0:
            ...
            if flag==1 and players_b[shomare].y>yp and flag_t==0:
                players_b[shomare].y=players_b[shomare].y-players_bv[shomare].vy
            if flag_ki==0 and flag==1:
                players_bv[shomare].vx,players_bv[shomare].vy,k2,h2,barkhordki,chandom,ki=barkhord(players_r , players_b,nobat,shomare,players_bv[shomare].vx,players_bv[shomare].vy,k2,h2,player_r_v,player_b_v)
            if flag_ki==1 and flag==1:
                players_bv[shomare].vx,players_bv[shomare].vy,k2,h2=barkhordkiy(players_r , players_b,nobat,shomare,players_bv[shomare].vx,players_bv[shomare].vy,k2,h2,player_r_v,player_b_v)
            toop_gg=barkhord_t_m(players_r,players_b,ball1,nobat,shomare)
            if barkhordki=="true":
                flag_ki=1
            if flag_ki==1 and ki=="Player2":
